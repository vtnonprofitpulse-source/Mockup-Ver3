...
export default async function handler(req, res) {
  try {
    ...
    let contentQuery = `SELECT c.id, c.organization_name, c.content_type, c.title, c.description, c.town, c.county, c.mission_area, c.source_url, c.event_date, c.status, 'content' as record_type FROM content c WHERE c.status = 'active'`;

    let orgsQuery = `SELECT o.id + 10000 as id, o.organization_name, 'Directory' as content_type, o.organization_name as title, o.mission_statement as description, o.town, o.county, o.mission_area, o.website_url as source_url, NULL as event_date, o.status, 'organization' as record_type FROM organizations o WHERE o.status = 'active' AND o.organization_name NOT IN (SELECT DISTINCT organization_name FROM content WHERE status = 'active')`;
