...
# === VALIDATION RULES ===
# Out-of-state cities to block
OUT_OF_STATE = [
    "new york city", "nyc", "park city", "boston", "portland", "chicago",
    "seattle", "denver", "los angeles", "san francisco", "miami",
    "washington dc", "philadelphia", "atlanta", "new hampshire", "maine",
    "massachusetts", "connecticut", "new york state"
]

# Generic phrases that indicate a bad description
BAD_PHRASES = [
...
# Generic titles that indicate Claude made something up
BAD_TITLE_PATTERNS = [
    "events -",
    "events (",
    "has an event",
...
def is_valid_record(record, org_name):
    ...
    # Must be valid content type
    if content_type not in VALID_TYPES:
        print(f"  BLOCKED: invalid type '{content_type}'")
        return False

    # Block out-of-state events
    for place in OUT_OF_STATE:
        if place in combined:
            print(f"  BLOCKED: out-of-state reference '{place}' in '{title}'")
            return False

    # Block bad descriptions
    for phrase in BAD_PHRASES:
        if phrase in description.lower():
            print(f"  BLOCKED: bad phrase '{phrase}' in description")
            return False

    # Block generic titles
    for pattern in BAD_TITLE_PATTERNS:
        if pattern in title.lower():
            print(f"  BLOCKED: generic title pattern '{pattern}'")
            return False
...
def is_duplicate(cursor, org_name, title, threshold=0.85):
    cursor.execute(
        "SELECT title FROM content WHERE organization_name = %s AND status = 'active'",
        (org_name,)
    )
    ...
    for existing_title in existing:
        ...
        if ratio >= threshold:
            print(f"  BLOCKED: fuzzy duplicate of '{existing_title}' (ratio: {ratio:.2f})")
            return True
    return False
...
def tag_content(raw_text, org_name, org_town):
    ...
    prompt = (
        ...
        "2. Skip any events in other states such as New York, Utah, Massachusetts, New Hampshire. "
...
def save_to_database(records, org_name, town, county, mission_area, source_url):
    ...
    for record in records:
        ...
        if not is_valid_record(record, org_name):
            blocked += 1
            continue
        # Run fuzzy duplicate check
